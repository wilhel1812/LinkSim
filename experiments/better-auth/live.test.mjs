...
test('live routes exclude fixture/linking and enforce origin, freshness and session revocation', async () => {
  ...
  try {
    ...
    // The actual library session freshness policy must protect enrollment and credential changes.
    ...
    assert.equal((await request('/api/auth/passkey/update-passkey', 'POST', headers, { id: 'missing', name: 'Stale rename' })).status, 403);
