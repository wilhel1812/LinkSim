  ...
  try {
    await db.batch([
      db.prepare(`INSERT INTO users
        ...
        VALUES (?, '', ?, '', '', ?, 1, '', 1, 0, 0, 1, ?, 'system:better-auth-registration', ?, ?)
      `).bind(linksimUserId, email, email, now, now, now),
      db.prepare(`INSERT INTO verified_identity_claims
        (normalized_email, current_user_id, status, created_at, updated_at)
        VALUES (?, ?, 'active', ?, ?)
      `).bind(email, linksimUserId, now, now),
      db.prepare(`INSERT INTO identity_subject_states
        ...
        VALUES (?, ?, 'current', ?, 1, ?, ?)
      `).bind(linksimUserId, email, linksimUserId, now, now),
      db.prepare(`INSERT INTO auth_identity_map (auth_user_id, linksim_user_id, created_at)
        VALUES (?, ?, ?)
      `).bind(input.authUserId, linksimUserId, now),
      db.prepare(`INSERT INTO user_identity_audit
        (event_type, target_user_id, source_user_id, actor_user_id, idp_email, details_json, created_at)
        VALUES ('better_auth_registration', ?, NULL, NULL, ?, ?, ?)
      `).bind(linksimUserId, email, JSON.stringify({ provider: "github", authUserId: input.authUserId }), now),
    ]);
